...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            let stmts = display_list(sequence_statements(command), "HI");
            const decls = filter(x => is_declaration(x), stmts);
            const others = filter(x => !is_declaration(x), stmts);
            let body = append(decls, others);
            C = is_null(body)
...
function parse_and_evaluate(string) {
    const parsed = display_list(parse(string));
    return evaluate(check(parsed));
}

function make_string(header, xs) {
    return header + "[" + accumulate((x, ys) => x + ", " + ys, "]", xs);
}
function check(parsed) {
    let declared = null;
    let not_declared = null;
    let access_before_initialisation = null;
    
    function find_names(t) {
        // display_list(t, "Search: ");
        if (is_name(t)) {
            const sym = symbol_of_name(t);
            if (is_null(member(sym, declared))) {
                not_declared = pair(sym, not_declared);
            }
        } else if (is_declaration(t)) {
            const sym = declaration_symbol(t);
            if (!is_null(member(sym, not_declared))) {
                not_declared = remove(sym, not_declared);
                access_before_initialisation = pair(sym, access_before_initialisation);
            }
            declared = pair(declaration_symbol(t), declared);
        } else if (is_list(t)) {
            while (!is_null(t)) {
                find_names(head(t));
                t = tail(t);
    ...
    display_list(declared, "SEARCH RESULTS Declared");
    display_list(not_declared, "SEARCH RESULTS Not");
    display_list(access_before_initialisation, "SEARCH RESULTS Before");
    if (!is_null(not_declared)) {
        const str1 = make_string("NOT DECLARED NAMES: ", not_declared);
        const str2 = make_string("ACCESS BEFORE INITIALISATION NAMES: ", access_before_initialisation);
...
`);
/*list(list("application", list("name", "f"), list(list("literal", 49))),
     list("function_declaration",
          list("name", "f"),
          list(list("name", "x")),
          list("return_statement", list("literal", 12))))*/